...
* This branch is for aligning R2Gaussians with a 3D model of an object and viduaslizing the X-ray by using 3D projection mapping.
...
[//]: # (* In cases where the X-ray and visual RGB data is taken of the same exact object, aligment can be performed manual points picked in cloud compare before using projections.)

[//]: # (* In other cases where the R2Gaussian is only a visual representative example &#40;not of the exact object on which projection is to be done&#41;, for example in the case of using 3D printed objects to project the X-ray onto, the alignment needs to be done in an ad-hoc manner in Blender.)
* In this README, we will go through the steps for both the above cases.
...
* This work was conducted while affiliated with the National Institute of Informatics in collaboration with [Watanabe Lab](https://www.vision.ict.e.titech.ac.jp/).

---

## Case-1:
1. The project starts with a 3D model of an object (Stanford Bunny) and a blender file containing stereo cameras. The blender file is provided by Watanabe Lab.
2. The goal is to align the R2Gaussian with the 3D model using manual points picked in cloud compare.
3. After this alignment, we can generate stereo image renders which can be used for projection mapping and visualizing the internal structure onto a 3D printed object (of the Stanford Bunny).
...
16. **Install Blender plugin for 3DGS**: Install Kiri's 3DGS plugin for Blender from [here](https://www.kiriengine.app/blender-addon/3dgs-render). This will allow you to visualize the aligned R2Gaussian in Blender.
17. **Import the file generated from step 15**: Import the aligned R2Gaussian file into Blender and adjust the colors and lighting to enhance the visualization. 
18. Recommended settings are: 
...
19. Render images as pleased. Enjoy!

## Case-2:

1. Foer this step, do steps  6-7 from Case-1.
2. Import the R2Gaussian into Blender using steps 16-17 from Case-1.
3. Manually transform the R2Gaussian in blender to make it align with the 3D model of the object.
4. Render using the settings mentioned in step 18.
---
